...
function sign(user, secret) {
  return jwt.sign(
    ...
    secret || "syncmasters-demo-secret",
    { expiresIn: "12h" }
  );
}
...
router.post("/register", async (c) => {
  try {
    ...
    return c.json({
      ...
      user,
      token: sign(user, c.env.JWT_SECRET)
    }, 201);
  } catch (e) {
...
router.post("/login", async (c) => {
  try {
    ...
    const now = Date.now();
    const attempts = loginAttempts.get(cleanEmail) || { count: 0, lockoutUntil: 0 };
    
    if (now < attempts.lockoutUntil) {
      const remainingMinutes = Math.ceil((attempts.lockoutUntil - now) / 60000);
      return c.json({ success: false, message: `Account locked. Try again in ${remainingMinutes} minute(s).` }, 429);
    }
    ...
    if (!user || !bcrypt.compareSync(password || "", user.password_hash)) {
      attempts.count += 1;
      let msg = "Invalid official credentials or password.";
      
      if (attempts.count >= 5) {
        attempts.lockoutUntil = now + 30 * 60 * 1000; // 30 minutes
        attempts.count = 0; // reset count for next lockout period
        msg = "Too many failed attempts. Account locked for 30 minutes.";
      } else {
        msg += ` You have ${5 - attempts.count} more chance(s).`;
      }
      
      loginAttempts.set(cleanEmail, attempts);
